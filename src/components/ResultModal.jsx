        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
             <div className="bg-background rounded-lg p-3 border border-border">
                ...
                <div className="text-sm font-semibold truncate text-white">{result.agentName || agent?.name}</div>
             </div>
             <div className="bg-background rounded-lg p-3 border border-border">
             ...
             <div className="bg-background rounded-lg p-3 border border-border">
                ...
                <div className="text-sm font-semibold text-white">{result.executionTime || '0.0s'}</div>
             </div>
             <div className="bg-background rounded-lg p-3 border border-border">
                <div className="text-[10px] uppercase tracking-wider text-muted mb-1">On-Chain Tx</div>
                {result.onChainTx ? (
                  <a href={`https://testnet-scan.helachain.com/tx/${result.onChainTx}`} target="_blank" rel="noreferrer" className="text-sm font-mono text-[#00FF94] hover:underline flex items-center gap-1 truncate" title={result.onChainTx}>
                    {result.onChainTx.slice(0,8)}... <ExternalLink size={12}/>
                  </a>
                ) : (
          ...
          <div className="mb-8">
            ...
            {formatAgentResult(agent?.category, result.result)}
          </div>
